...
The MCP adapter exposes `create_blocked_goal_recovery`, `consume_blocked_goal_recovery`, `record_blocked_goal_recovery_decision`, `prepare_blocked_goal_resume_attempt`, `intake_blocked_goal_resume_evidence`, `review_blocked_goal_resume_evidence`, `close_blocked_goal_resume_attempt`, and `validate_blocked_goal_recovery_lifecycle`. Each tool accepts only `inputDir` plus optional contained `outputDir`, delegates to `@hardening-mcp/acceptance`, and returns a stage-bound `repoassure.mcp-blocked-goal-recovery-tool-result.v1` with explicit no-command, no-external-state, and no-target-repo-mutation evidence. Calls are locally destructive because fixed-name evidence files may be replaced; non-blocking no-follow regular-file reads with an 8 MiB cap, directory device/inode guards, output-symlink rejection, and atomic output replacement enforce the selected local directory boundary.

## Blocked Goal Recovery MCP Real Client Consumption Validation v0.1

`tests/support/real-mcp-client.ts` starts the compiled server through the official SDK `StdioClientTransport`, captures a redacted bounded stderr tail, applies bounded initialization requests, and deterministically terminates the observed child PID on normal close or failed connect. `tests/integration/mcp-real-client.test.ts` verifies initialize, instructions, all eight tool discovery, seven reviewed artifact stages, lifecycle error consumption, output-schema validation, safe environment inheritance, `content`/`structuredContent` agreement, missing-input and argument errors, early exit, timeout, and cleanup. The near-real eight-outcome campaign executes successful lifecycle validation through the same real client. MCP errors omit `structuredContent` so success-only output schemas do not hide client-readable failures.
