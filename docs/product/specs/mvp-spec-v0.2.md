...
安全边界决策：ADR-0013 将 Codex Security 视为优先集成的 security provider，而不是 RepoAssure 的直接替代品或唯一依赖。v0.2 仍聚焦 acceptance、repair plan、repair handoff、repair execution 和 patch plan；Security Assurance Lane Phase 1 已作为可选 provider-backed 增量支持本地扫描目录导入、脱敏、provider provenance 和 repair planning 输入，但不成为当前 MVP 必需验收门槛。
...
v0.2 也不是通用安全扫描器。Codex Security、CodeQL、Semgrep、Gitleaks、OSV 等安全工具的输出应通过 Security Assurance Lane 导入、脱敏、保留 provider provenance，并转化为 RepoAssure artifact / repair planning 输入；当前已实现 Phase 1 本地 provider scan directory 导入 MVP，但不自研 deep vulnerability scanner、不调用 scanner runtime、不联网、不上传目标 repo。
...
- 安全审计、性能优化或合规证明的完整替代。
- Codex Security、CodeQL、Semgrep、Gitleaks、OSV 等 provider 的完整扫描执行、远端同步和安全平台替代；Security Assurance Lane Phase 1 仅导入本地 provider 输出并生成 RepoAssure artifacts / repair planning 输入。
- 自研完整 deep vulnerability scanner 或替代 Codex Security。
- 对非 Web 技术栈的全面支持。
...
- Security Assurance Lane：Phase 1 已支持导入本地 Codex Security fixture-style provider 输出，下一阶段可扩展更多 provider 格式、SARIF 兼容和 provider coverage 审计。
