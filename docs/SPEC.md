...
The schema is `repoassure.blocked-goal-recovery-decision-receipt.v1`. Required sections are `decisionStatus`, `resumeAttemptReadiness`, `sourceConsumptionReport`, `decisionSummary`, `decisionItems`, approved/rejected/deferred/risk-accepted action sets, `reviewedResumeCommands`, `boundaryCompliance`, `maintainerReviewBoundary`, `redactionBoundary`, `nonAuthorizationBoundary`, and `blockedActions`. Source SHA-256 hashes raw file bytes. Duplicate and unknown action keys are invalid; missing decisions remain unreviewed. The receipt does not execute resume commands.
