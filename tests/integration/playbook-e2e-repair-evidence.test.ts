...
describe('AI IDE repair evidence end-to-end campaign fixture', () => {
  it('carries a local campaign summary through playbook, decisions, approval, plan, and evidence artifacts', async () => {
    ...
    const resumeTaskText = await readFile(join(outputDir, 'blocked-goal-recovery-resume-attempt-task-package.json'), 'utf8');
    ...
    const resumeReviewText = await readFile(
      join(outputDir, 'blocked-goal-recovery-resume-attempt-evidence-review-decision-package.json'), 'utf8'
    );
    const resumeReview = JSON.parse(resumeReviewText) as {
      reviewItems: Array<{ evidenceKey: string; decision: string }>;
    };
    await writeFile(join(outputDir, 'blocked-goal-recovery-resume-attempt-closure-input.json'), `${JSON.stringify({
      sourceEvidenceReviewPackageSha256: createHash('sha256').update(resumeReviewText).digest('hex'),
      closureEvidence: 'Fixture closure reviewed.', reviewerRole: 'maintainer',
      acknowledgedRiskEvidenceKeys: resumeReview.reviewItems
    ...
    expect(outputs.blockedGoalRecoveryResumeAttemptTaskPackage.schemaVersion).toBe('repoassure.blocked-goal-recovery-resume-attempt-task-package.v1');
    expect(outputs.blockedGoalRecoveryResumeAttemptTaskPackage.taskPackageStatus).toBe('ready_for_separate_resume_attempt');
    expect(outputs.blockedGoalRecoveryResumeAttemptTaskPackage.resumeCommandTasks).toEqual([
      expect.objectContaining({ command: 'codex resume goal', executed: false })
    ]);
    expect(outputs.blockedGoalRecoveryResumeAttemptTaskPackage.boundaryCompliance.commandsExecuted).toBe(false);
    expect(outputs.blockedGoalRecoveryResumeAttemptTaskPackage.blockedActions).toContain('public_launch');
    ...
    expect(outputs.blockedGoalRecoveryResumeAttemptClosure.schemaVersion).toBe('repoassure.blocked-goal-recovery-resume-attempt-closure-receipt.v1');
    expect(outputs.blockedGoalRecoveryResumeAttemptClosure.closureStatus).toBe('closed');
...
async function readArtifacts(outputDir: string): Promise<{
  ...
  blockedGoalRecoveryResumeAttemptTaskPackage: {
    schemaVersion: string;
    taskPackageStatus: string;
    resumeCommandTasks: Array<{ command: string; executed: boolean }>;
    boundaryCompliance: { commandsExecuted: boolean; sourceBoundaryPreserved: boolean };
    blockedActions: string[];
  };
  blockedGoalRecoveryResumeAttemptEvidenceIntake: {
    schemaVersion: string;
    intakeStatus: string;
  ...
  blockedGoalRecoveryResumeAttemptClosure: {
    schemaVersion: string; closureStatus: string;
    boundaryCompliance: {
  ...
  return {
    ...
    blockedGoalRecoveryResumeAttemptTaskPackage: JSON.parse(await readFile(join(outputDir, 'blocked-goal-recovery-resume-attempt-task-package.json'), 'utf8')) as {
      schemaVersion: string;
      taskPackageStatus: string;
      resumeCommandTasks: Array<{ command: string; executed: boolean }>;
      boundaryCompliance: { commandsExecuted: boolean; sourceBoundaryPreserved: boolean };
    ...
    blockedGoalRecoveryResumeAttemptClosure: JSON.parse(await readFile(join(outputDir, 'blocked-goal-recovery-resume-attempt-closure-receipt.json'), 'utf8')) as {
      schemaVersion: string; closureStatus: string;
      boundaryCompliance: {
        commandsExecutedByClosure: boolean; externalGoalClosedByReceipt: boolean; sourceBoundaryPreserved: boolean;
      };
