...
describe('project intelligence watch mode', () => {
  it('watches bounded source and governance files while ignoring generated, dependency, cache, and secret paths', () => {
    ...
    expect(shouldRefreshProjectIntelligencePath('docs/logs/dev-log.md')).toBe(true);
    ...
    /* The local goal workspace is not source and is git-ignored, so a write there
       must not trigger a refresh. */
    expect(shouldRefreshProjectIntelligencePath('.autopilot/progress/snapshot.json')).toBe(false);
    expect(shouldRefreshProjectIntelligencePath('.autopilot/goals/example.md')).toBe(false);
  });
