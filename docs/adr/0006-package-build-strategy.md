...
- Do not broaden Phase 2d into `core` or `browser-explorer` extraction without a separate TDD goal.
...
The acceptance package pilot is no longer deferred. Phase 2c shared package extraction is implemented with compatibility wrappers. Phase 2d repair-planner package extraction is implemented with compatibility wrappers. Broader package extraction for `browser-explorer` and `core` remains deferred until each package has equivalent compatibility-output and import-boundary tests.
...
Package build order is now package-first for the acceptance, shared, and repair-planner packages: `pnpm build` runs `build:shared`, `build:repair-planner`, `build:acceptance`, then `build:src`, and `pnpm typecheck` builds package declarations before checking root `src`. This lets legacy `src/internal/acceptance/*`, `src/shared/*`, `src/domain/repair-plan/*`, and `src/types/repair-plan.ts` compatibility wrappers re-export package dist APIs without violating the root `src` build `rootDir`.
