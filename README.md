...
- Blocked Goal Recovery Consumption Validation v0.1：`pnpm goal:recover:consume -- --from-dir <dir>` 会读取本地 `blocked-goal-recovery-package.json`，生成 `blocked-goal-recovery-consumption-report.json` / `.md`，schema 为 `repoassure.blocked-goal-recovery-consumption-report.v1`。报告提供 evidence read order、resume readiness、automatic retry / maintainer decision / external prerequisite action queue 和 resume checklist；它不执行 recovery commands，也不授权目标 repo 修改、发布、launch、客户联系、pricing/spend 或商业版 availability claims。
