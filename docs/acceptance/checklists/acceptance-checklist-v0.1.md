...
- [x] Public Website Custom Domain Deployment v0.1
  - 当前状态：新增 `docs/operations/public-website-custom-domain-deployment-v0.1.md`，记录 `repoassure.com` / `www.repoassure.com` 自定义域名部署尝试；`pnpm build:website` 和 `wrangler pages deploy` 已通过，Pages custom domain API 已接受两个域名绑定，但当前状态为 `blocked_dns_cname_not_set`，因为 Cloudflare Pages 返回 `CNAME record not set` 且当前 API token 写 DNS 返回 `Authentication error`。HTTPS、内容 smoke、语言切换和 forbidden-claim custom-domain verification 仍未完成；不授权 public launch、repo public、npm publication、GitHub release、SaaS/Team Cloud/Enterprise 或 hosted dashboard availability claims。
