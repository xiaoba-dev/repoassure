...
Private Preview Reviewer Handoff Dispatch & Feedback Intake Readiness 的测试范围限于 structure-level tests：`project-structure.test.ts` 守护 dispatch readiness 文档存在，并要求 reviewer handoff、triage/backlog、release candidate handoff、acceptance checklist、testing strategy 和 dev log 级联记录 `waiting_for_reviewer_feedback`、`reviewer1@example.com`、`reviewer2@example.com`、handoff message template、feedback intake record template、不发送邮件、不创建外部 issue、不编造 reviewer feedback，以及不记录 OTP/cookie/Access token/login query-state 的边界。该 gate 不验证真实反馈内容；真实反馈收到后必须另开 triage execution goal。
