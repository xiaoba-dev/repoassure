...
Protected PR Workflow Verification v0.1 的测试覆盖 structure-level tests、本地质量门和远端 PR workflow verification：`project-structure.test.ts` 守护 `docs/operations/protected-pr-workflow-verification-v0.1.md`、README、acceptance checklist、testing strategy、decision log 和 dev log 的级联，要求状态为 `pr_created_ci_pending_or_passed`，记录 workflow branch、base branch、branch protection remains enabled、required status check `Quality Gates`、pull request workflow、review gate、禁止弱化 branch protection、禁止直推 `main` 和禁止发布动作。远端验证通过 GitHub PR、`RepoAssure CI` / `Quality Gates`、review gate 和 merge gate 完成。
