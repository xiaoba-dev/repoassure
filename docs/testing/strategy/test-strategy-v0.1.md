...
- Unit: `tests/unit/security-assurance.test.ts` covers the six-provider catalog, exact normalized schema, object-valued findings, required and optional field typing, strict severity, P0-P3 repair task mapping, regular/non-symlink/10 MiB input, safe filesystem errors, collision-resistant secret and ordinary finding/task ids, metadata redaction, repo-local `.hardening` containment, symbolic-link rejection, create-only evidence, whole-artifact trust-boundary ordering, Markdown/HTML literal rendering, bare URL/email autolink neutralization, adversarial prompt neutralization, and provider verification as untrusted review evidence. `tests/unit/cli-options.test.ts` and `tests/unit/mcp-tool-registry.test.ts` additionally prove callers cannot bypass the output boundary and receive path-safe `run_dir_invalid` errors.
