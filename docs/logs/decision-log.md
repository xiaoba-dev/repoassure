...
## 2026年7月14日 - Security Assurance Lane provider import ergonomics

### 决策

在 ADR-0013 的既有边界内接受共享 provider catalog、稳定预检错误、CLI/MCP discoverability 与 repair-planning handoff。该实现不新增 ADR，因为没有改变 provider-backed、local-first、no scanner、no target mutation 的产品架构决议。

### 原因

- 旧实现可以导入 fixture，但 provider 能力声明分散，CLI help 容易被误读成原生 provider format support，MCP 无 discover/import surface。
- AI IDE 需要稳定错误码和下一步 repair-plan 参数，而不是解析原始 `ENOENT` 或猜测 artifact 顺序。
- 一个 package-owned contract 可以防止 CLI、MCP 和 importer 的 provider 列表漂移。
...
## 2026-07-13 - blocked goal recovery decision receipt

### 决策

接受 Blocked Goal Recovery Decision Receipt v0.1 和 ADR-0035。Recovery consumption action queue 使用稳定 `actionKey`，maintainer 通过 `goal:recover:decide` 记录 approve/reject/defer/accept_risk 决策，缺失决定阻塞 separate resume attempt。

### 边界

Receipt does not execute resume commands，不授权 target repo mutation、release、launch、customer contact、pricing/spend、repository visibility 或 commercial/hosted availability claims。

## 2026-07-13 - blocked goal recovery resume attempt task package

### 决策

接受 Blocked Goal Recovery Resume Attempt Task Package v0.1 和 ADR-0036。Ready decision receipt 必须先转换成 exact-byte-bound、可审计的 action/command task queue，AI IDE 不从聊天上下文推断执行范围。
