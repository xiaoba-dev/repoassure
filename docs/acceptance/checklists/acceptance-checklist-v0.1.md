...
- [x] Private Preview Reviewer Handoff Dispatch & Feedback Intake Readiness
  - 当前状态：新增 `docs/operations/private-preview-reviewer-handoff-dispatch-readiness-v0.1.md`，handoff scope 已更新为真实 reviewer emails `confirmed-reviewer-1` 和 `confirmed-reviewer-2`；`reviewer1@example.com` 和 `reviewer2@example.com` 仅作为历史 placeholder 记录保留。状态为 `waiting_for_reviewer_feedback`。该项不发送邮件、不创建外部 issue、不编造 reviewer feedback、不记录 OTP、cookie、Access token、login query-state、raw Access redirect URL、reviewer credentials 或无关个人数据。
- [x] Private Preview Reviewer Identity Reconciliation
  - 当前状态：新增 `docs/operations/private-preview-reviewer-identity-reconciliation-v0.1.md`，澄清 Maintainer / user、authenticated reviewer identity `web3coderman@gmail.com` 和 placeholder second-batch reviewer emails 的边界；`reviewer1@example.com` 与 `reviewer2@example.com` 当前为 placeholder only，必须替换为真实 reviewer emails 后才能作为外部 reviewer feedback 输入。该项不修改 Cloudflare Access policy、不发送邀请、不编造 reviewer feedback、不记录 OTP、cookie、Access token、login query-state 或 reviewer credentials。
- [x] Private Preview Real Reviewer Replacement
  - 当前状态：新增 `docs/operations/private-preview-real-reviewer-replacement-v0.1.md`，在用户明确确认保存后通过 Cloudflare Dashboard UI 更新 `RepoAssure reviewer allow`，移除 placeholder reviewer emails `reviewer1@example.com`、`reviewer2@example.com`，并确认真实 reviewer emails `confirmed-reviewer-1`、`confirmed-reviewer-2`；`pnpm verify:cloudflare-preview` 仍应返回预期 `manual_required`，因为登录后 reviewer smoke 不能自动绕过 email/OTP。该项不发送 reviewer invitation、不创建 external issue、不编造 reviewer feedback、不记录 OTP、cookie、Access token、login query-state、raw Access redirect URL、reviewer credentials 或无关个人数据。
- [x] Private Preview Reviewer Handoff Package and Dispatch Execution
  - 当前状态：新增 `docs/operations/private-preview-reviewer-handoff-package-and-dispatch-execution-v0.1.md`，Stage 1 handoff package 已生成，Stage 2 dispatch execution 因未确认发送渠道保持 `pending_channel_confirmation`；使用 `confirmed-reviewer-1`、`confirmed-reviewer-2` 匿名槽位，不在 Git tracked docs 记录 reviewer PII。No outbound message was sent，不创建 external issue，不编造 reviewer feedback，不记录 OTP、cookie、Access token、login query-state、raw Access redirect URL、reviewer credentials 或无关个人数据。
