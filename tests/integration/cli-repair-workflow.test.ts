...
describe('repair workflow CLI', () => {
  it('generates the complete no-write repair evidence chain', async () => {
    ...
    const evidenceOutput = join(handoffOutput, 'evidence-package');
    ...
    const handoffPackage = JSON.parse(await readFile(handoff.packagePath, 'utf8')) as {
      ...
      repairActionQueue: Array<{ taskId: string }>;
    };
    const taskId = handoffPackage.repairActionQueue[0]?.taskId;

    expect(handoff.taskCount).toBe(1);
    expect(taskId).toBeTruthy();
    expect(handoffPackage.agentContract.readOrder).toContain('repairActionQueue[]');
    ...
    expect(handoffPackage.maintainerReview.requiredBefore).toContain('applying patches');
    ...
    expect(dryRunReport.noWriteProof).toMatchObject({
    ...
    expect(patchPlanDocument.noWriteProof).toMatchObject({
      targetRepoWriteAuthorized: false,
      patchesApplied: false
    });
    expect(patchPlanDocument.maintainerReview.allowedDecisions).toEqual([
      'approve',
      'reject',
      'defer',
      'accept_risk'
    ...
    const evidenceResult = await runCliForTest([
      'repair',
      'evidence-package',
      '--handoff',
      handoff.packagePath,
    ...
    const evidence = JSON.parse(evidenceResult.stdout) as { packagePath: string };
    const evidencePackage = JSON.parse(await readFile(evidence.packagePath, 'utf8')) as {
      agentContract: {
        readOrder: string[];
        nextCommands: { rerunValidation: string; regeneratePatchPlan: string };
      };
      repairFlow: Array<{ stage: string }>;
    };
    expect(evidencePackage.agentContract.readOrder).toContain('artifactIndex');
    expect(evidencePackage.agentContract.nextCommands).toEqual({
