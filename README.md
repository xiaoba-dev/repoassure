...
分支保护与发布边界见 `docs/operations/branch-protection-release-boundary-v0.1.md`。当前目标状态是 `main` 要求 `RepoAssure CI` / `Quality Gates`，但 GitHub 对当前 private repo plan 返回 403；仓库仍必须保持 private，不能发布 package、移除 `package.json` `"private": true`、公开仓库或通过公开仓库绕过该限制。仓库级 Apache-2.0 `LICENSE` 只是 public-release readiness material，不是公开发布授权。Release candidate 本地打包与审查交接见 `docs/operations/release-candidate-handoff-v0.1.md`；Public Website release candidate 审查交接见 `docs/operations/public-website-release-candidate-handoff-v0.1.md`。这些交接同样不授权 push、PR、GitHub release、npm publish、部署官网或仓库公开。
