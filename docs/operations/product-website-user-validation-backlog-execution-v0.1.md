...
| `automatable_next` | Security Assurance Lane Phase 1 provider import ergonomics | The Phase 1 local provider import exists; its CLI/MCP discoverability and error guidance need a focused usability review. | `docs/architecture/specs/security-assurance-lane-spec-v0.1.md`; `packages/security-assurance/` | Execute `Security Assurance Lane Provider Import Ergonomics v0.1` with TDD and local fixtures only. |
...
`Security Assurance Lane Provider Import Ergonomics v0.1` is the next automatable goal. It will assess and improve the local provider-import path using synthetic or fixture evidence only. It must not invoke a security scanner service, upload source, alter a target repository, publish a package, or claim hosted/commercial availability.
