...
安全边界决策：ADR-0013 将 Codex Security 视为未来优先集成的 security provider，而不是 RepoAssure 的直接替代品或唯一依赖。v0.2 仍聚焦 acceptance、repair plan、repair handoff、repair execution 和 patch plan；Security Assurance Lane 属于后续 provider-backed 增量。
...
v0.2 也不是通用安全扫描器。Codex Security、CodeQL、Semgrep、Gitleaks、OSV 等安全工具的输出未来应通过 Security Assurance Lane 导入、脱敏、保留 provider provenance，并转化为 RepoAssure artifact / repair planning 输入；当前 v0.2 不自研 deep vulnerability scanner。
...
- 安全审计、性能优化或合规证明的完整替代。
- Codex Security、CodeQL、Semgrep、Gitleaks、OSV 等 provider 的安全证据导入；该能力由 ADR-0013 定义为后续 Security Assurance Lane。
...
- Security Assurance Lane：导入 Codex Security 等 provider 输出，规范化为本地 artifact 和 repair planning 输入。
