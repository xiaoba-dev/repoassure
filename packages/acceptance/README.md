...
- Report the three process-record goal audit items (`开发流程`, `日志治理`, `Token 控制`) as `enforcement: 'advisory'` per ADR-0042: they stay in the audit document and keep naming their missing markers, but they count into `GoalAuditSummary.advisory` instead of `missing`, so only blocking items decide the `goal:audit` and `user:handoff` exit codes.
