#!/usr/bin/env node
import { writeBlockedGoalRecoveryResumeAttemptEvidenceReviewDecisionPackageFromDirectory } from '../packages/acceptance/dist/blocked-goal-recovery-resume-attempt-evidence-review-decision-package.js';
import { redactSensitiveText } from '../packages/acceptance/dist/redaction.js';

function help() {
  console.log(`RepoAssure recovery resume evidence review decision package

Usage:
  pnpm --silent goal:recover:review-resume-evidence -- --from-dir <dir> [--output <dir>]
`);
}
try {
  ...
  const result = await writeBlockedGoalRecoveryResumeAttemptEvidenceReviewDecisionPackageFromDirectory({
