...
ADR-0038 固化 Blocked Goal Recovery Resume Attempt Evidence Review Decision Package v0.1：maintainer acceptance 必须逐项、可审计且不触发命令执行。ADR-0039 固化 Blocked Goal Recovery Resume Attempt Closure Receipt v0.1：只有 accepted / accepted-with-risk review 可生成 exact-source-bound 本地 closure，且不关闭 external goal。
