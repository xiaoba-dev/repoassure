...
ADR-0020 定义 public website private preview deployment boundary：官网代码进入 `main` 不等于部署或公开发布。Private preview deployment、production deployment 和 public launch 是三个独立 gate；当前只完成部署规划，deployment execution 需要单独 Codex goal，且必须记录 hosting target、access control、secret handling、rollback、smoke verification 和 post-deployment boundaries。
