...
- MCP Real Client Consumption Validation v0.1：`pnpm test:mcp-real-client` 使用官方 SDK `Client` + `StdioClientTransport` 启动真实 MCP 子进程，验证与具体工具无关的 transport 契约：bounded initialization timeout、子进程 PID 的确定性清理、SDK 安全默认环境隔离。工具发现由 `pnpm test:mcp-external-config` 按精确工具集断言。错误结果只返回标准 text `content` + `isError: true`，避免 success output schema 拒绝 `{ error }` structuredContent；该验证不修改目标 repo。
- Blocked Goal Recovery MCP External AI IDE Configuration Validation v0.1：`pnpm --silent mcp:config -- --client <cursor|vscode|codex>` 生成本地 stdio 配置，固定使用当前 Node.js 绝对路径和 `apps/mcp-server/index.js`；`pnpm test:mcp-external-config` 从 repo 外部、带空格 source-checkout alias 启动三种配置并断言精确的产品工具集、SDK harness 环境隔离和进程清理。新空配置可直接使用；已有配置必须只合并 `repoassure` entry。生成器 does not write client configuration，验证 does not modify target repository source；真实 IDE 环境继承留给人工验收。
- MCP Real AI IDE Manual Acceptance v0.1：维护者可按 `docs/operations/blocked-goal-recovery-mcp-real-ai-ide-manual-acceptance-v0.1.md` 使用一个真实安装的客户端检查工具发现，并只对 disposable 目录调用一次非改写的 `analyze_repo`。2026-07-14 Codex 的人工验收针对的是已被 ADR-0044 移除的 recovery MCP 面，脱敏证据作为历史记录保留、不再代表当前面的验收；更新后的 runbook 尚未重跑。这不代表发布、目标仓库修改或 hosted/commercial availability。
