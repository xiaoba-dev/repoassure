...
ADR-0005 accepts a phased monorepo migration. The initial Phase 2 package extraction was deferred until the build could preserve compatibility outputs, but the acceptance package pilot is no longer deferred: `packages/acceptance` now owns acceptance implementation modules and runner entrypoints while compatibility outputs remain available under `src/internal/acceptance/*` and `dist/internal/acceptance/*`. Phase 2c shared package extraction is also implemented: `packages/shared` now owns redaction and shell helper implementations while compatibility outputs remain available under `src/shared/*` and `dist/shared/*`. Phase 2d repair-planner package extraction is implemented: `packages/repair-planner` now owns repair plan and executable repair task package implementation modules while compatibility outputs remain available under `src/domain/repair-plan/*`, `dist/domain/repair-plan/*`, `src/types/repair-plan.ts`, and `dist/types/repair-plan.*`. Phase 2e browser-explorer package extraction is implemented: `packages/browser-explorer` now owns browser/fetch exploration and Playwright driver implementation modules while compatibility outputs remain available under `src/domain/explore/*` and `dist/domain/explore/*`. Existing CLI bins, MCP entrypoints, acceptance scripts, goal audit, benchmark runs, tests, and documentation still rely on stable compatibility paths.
...
The first package extraction target is `packages/acceptance`, because its boundary is narrower than `core` and less coupled than the earlier root shared utility surface. Phase 2c applies the same strategy to `packages/shared` after package-first build order and `dist/shared/*` compatibility wrappers are tested. Phase 2d applies the same strategy to `packages/repair-planner` after repair plan behavior parity, package export, type-smoke, and legacy `dist/domain/repair-plan/*` plus `dist/types/repair-plan.*` compatibility tests are in place. Phase 2e applies the same strategy to `packages/browser-explorer` after explorer behavior parity, package export, type-smoke, and legacy `dist/domain/explore/*` compatibility tests are in place.
...
- Keep `dist/domain/repair-plan/*` and `dist/types/repair-plan.*` JavaScript and declaration outputs available as compatibility surfaces until downstream callers no longer rely on them.
- Keep `dist/domain/explore/*` JavaScript and declaration outputs available as compatibility surfaces until downstream callers no longer rely on them.
- Update ADR-0005 and the monorepo structure spec as each package extraction phase completes.
- Do not broaden Phase 2e into `core` extraction without a separate TDD goal.
...
The acceptance package pilot is no longer deferred. Phase 2c shared package extraction is implemented with compatibility wrappers. Phase 2d repair-planner package extraction is implemented with compatibility wrappers. Phase 2e browser-explorer package extraction is implemented with compatibility wrappers. Broader package extraction for `core` remains deferred until it has equivalent compatibility-output and import-boundary tests.
...
Package build order is now package-first for the acceptance, shared, browser-explorer, and repair-planner packages: `pnpm build` runs `build:shared`, `build:browser-explorer`, `build:repair-planner`, `build:acceptance`, then `build:src`, and `pnpm typecheck` builds package declarations before checking root `src`. This lets legacy `src/internal/acceptance/*`, `src/shared/*`, `src/domain/explore/*`, `src/domain/repair-plan/*`, and `src/types/repair-plan.ts` compatibility wrappers re-export package dist APIs without violating the root `src` build `rootDir`.
