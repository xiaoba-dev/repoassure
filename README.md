...
Cloudflare Access remote preview preflight 可通过 `pnpm preflight:cloudflare-preview` 生成本地 evidence，交接见 `docs/operations/cloudflare-access-preview-preflight-v0.1.md`。该预检不上传 website source/build output、不调用 Cloudflare API、不创建 preview URL；Cloudflare Pages preview deployments are public by default，因此 Cloudflare Access policy must be enabled before any preview URL is shared。Public Website Custom Domain Deployment v0.1 见 `docs/operations/public-website-custom-domain-deployment-v0.1.md`，当前状态为 `verified_custom_domain_active`：`repoassure.com` 和 `www.repoassure.com` 均已在 Cloudflare Pages custom domains 中 active，HTTPS 返回 200，`pnpm verify:website` 已分别通过两个 custom domain 的英文/中文、桌面/移动、Trust Ledger、Assurance Graph、artifact tabs、private preview form 和 forbidden-claim 验证；该记录不授权仓库公开、npm publish、GitHub release、public launch、production marketing announcement、SaaS/Team Cloud/Enterprise 或 hosted dashboard availability claims。
