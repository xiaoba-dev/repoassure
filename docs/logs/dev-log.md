...
## 2026年7月6日 - AI IDE Playbook Consumption Dry-Run Report v0.1

### 完成内容

- 新增 `packages/acceptance/src/ai-ide-playbook-consumption-report.ts`。
- 新增 `repoassure.ai-ide-playbook-consumption-report.v1`，从本地 `ai-ide-repair-playbook.json` 生成 dry-run understanding report。
- 报告输出 `campaignUnderstanding`、`repairTaskUnderstanding`、`readOrderCompliance`、`dryRunDecision.blockedActions`、maintainer review boundary、redaction boundary 和 non-authorization boundary。
- 新增 `ai-ide-playbook-consumption-report.json` / `ai-ide-playbook-consumption-report.md` 输出。
- 新增 `pnpm playbook:consume` 和 `scripts/generate-ai-ide-playbook-consumption-report.mjs`。
- 新增 `tests/integration/playbook-consume.test.ts`，验证 CLI smoke、schema、Markdown 和敏感路径脱敏。
- 新增 `docs/operations/ai-ide-playbook-consumption-dry-run-report-v0.1.md`，并级联更新 README、acceptance checklist、testing strategy 和 decision log。

### TDD 记录

- Red：先新增 `tests/unit/ai-ide-playbook-consumption-report.test.ts`，测试因 `packages/acceptance/src/ai-ide-playbook-consumption-report.ts` 缺失按预期失败。
- Green：实现最小 report builder、Markdown renderer、writer、package export 和 type-smoke 子路径覆盖。
- Red：新增 `tests/integration/playbook-consume.test.ts`，测试因 `pnpm playbook:consume` 不存在按预期失败。
...
## 2026年7月6日 - AI IDE Repair Decision Package v0.1

### 完成内容

- 新增 `packages/acceptance/src/ai-ide-repair-decision-package.ts`。
- 新增 `repoassure.ai-ide-repair-decision-package.v1`，从本地 `ai-ide-playbook-consumption-report.json` 生成 maintainer repair decision package。
- 决策包输出 `decisionSummary`、`decisionItems`、`targetReviewSummary`、`maintainerDecisionChecklist`、inherited blocked actions、maintainer review boundary、redaction boundary 和 non-authorization boundary。
- 新增 `ai-ide-repair-decision-package.json` / `ai-ide-repair-decision-package.md` 输出。
- 新增 `pnpm playbook:decide` 和 `scripts/generate-ai-ide-repair-decision-package.mjs`。
- 新增 `tests/integration/playbook-decide.test.ts`，验证 CLI smoke、schema、Markdown 和敏感路径脱敏。
- 新增 `docs/operations/ai-ide-repair-decision-package-v0.1.md`，并级联更新 README、acceptance checklist、testing strategy 和 decision log。

### TDD 记录

- Red：先新增 `tests/unit/ai-ide-repair-decision-package.test.ts`，测试因 `packages/acceptance/src/ai-ide-repair-decision-package.ts` 缺失按预期失败。
- Green：实现最小 decision package builder、Markdown renderer、writer、package export 和 type-smoke 子路径覆盖。
- Red：新增 `tests/integration/playbook-decide.test.ts`，测试因 `pnpm playbook:decide` 不存在按预期失败。
- Green：新增 CLI 脚本和 package script。
- Red：更新 `tests/unit/project-structure.test.ts`，要求本 operation 文档、脚本入口和级联记录存在；测试因 operation 文档缺失按预期失败。
- Green：新增 operation packet 和级联文档。
...
## 2026年7月6日 - AI IDE Approved Repair Execution Plan v0.1

### 完成内容

- 新增 `packages/acceptance/src/ai-ide-approved-repair-execution-plan.ts`。
- 新增 `repoassure.ai-ide-approved-repair-execution-plan.v1`，从本地 `ai-ide-repair-approval-receipt.json` 生成 approved repair execution plan。
- execution plan 只纳入 `approvalDecision=approve` 且 `decisionType=manual_repair_candidate` 的 approval receipt item。
- execution plan 输出 `executionSummary`、`approvedExecutionItems`、`excludedApprovalItems`、`executionChecklist`、`rollbackAndReviewChecklist`、blocked actions、maintainer review boundary、redaction boundary 和 non-authorization boundary。
- 新增 `ai-ide-approved-repair-execution-plan.json` / `ai-ide-approved-repair-execution-plan.md` 输出。
- 新增 `pnpm playbook:plan-approved` 和 `scripts/generate-ai-ide-approved-repair-execution-plan.mjs`。
- 新增 `tests/integration/playbook-plan-approved.test.ts`，验证 CLI smoke、schema、Markdown 和敏感路径脱敏。
- 新增 `docs/operations/ai-ide-approved-repair-execution-plan-v0.1.md`，并级联更新 README、acceptance checklist、testing strategy 和 decision log。

### TDD 记录

- Red：先新增 `tests/unit/ai-ide-approved-repair-execution-plan.test.ts`，测试因 `packages/acceptance/src/ai-ide-approved-repair-execution-plan.ts` 缺失按预期失败。
- Green：实现最小 approved repair execution plan builder、Markdown renderer、writer、package export 和 type-smoke 子路径覆盖。
- Red：新增 `tests/integration/playbook-plan-approved.test.ts`，测试因 `pnpm playbook:plan-approved` 不存在按预期失败。
- Green：新增 CLI 脚本和 package script。
- Red：更新 `tests/unit/project-structure.test.ts`，要求本 operation 文档、脚本入口和级联记录存在；测试因 operation 文档缺失按预期失败。
- Green：新增 operation packet 和级联文档。
...
## 2026年7月7日 - AI IDE Repair Execution Evidence Report v0.1

### 完成内容

- 新增 `packages/acceptance/src/ai-ide-repair-execution-evidence-report.ts`。
- 新增 `repoassure.ai-ide-repair-execution-evidence-report.v1`，从本地 `ai-ide-approved-repair-execution-plan.json` 和 repair execution evidence input 生成 evidence report。
- evidence report 记录 approved execution item 的 `executionStatus`、`readOrderCompliance`、verification results、maintainer review status、boundary report 和 blocked actions。
- 新增 `ai-ide-repair-execution-evidence-report.json` / `ai-ide-repair-execution-evidence-report.md` 输出。
- 新增 `pnpm playbook:evidence` 和 `scripts/generate-ai-ide-repair-execution-evidence-report.mjs`。
- 新增 `tests/integration/playbook-evidence.test.ts`，验证 CLI smoke、schema、Markdown 和敏感路径脱敏。
- 新增 `docs/operations/ai-ide-repair-execution-evidence-report-v0.1.md`，并级联更新 README、acceptance checklist、testing strategy 和 decision log。

### TDD 记录

- Red：先新增 `tests/unit/ai-ide-repair-execution-evidence-report.test.ts`，测试因 `packages/acceptance/src/ai-ide-repair-execution-evidence-report.ts` 缺失按预期失败。
- Green：实现最小 repair execution evidence report builder、Markdown renderer、writer、package export 和 type-smoke 子路径覆盖。
- Red：新增 `tests/integration/playbook-evidence.test.ts`，测试因 `pnpm playbook:evidence` 不存在按预期失败。
- Green：新增 CLI 脚本和 package script。
- Red：更新 `tests/unit/project-structure.test.ts`，要求本 operation 文档、脚本入口和级联记录存在；测试因 operation 文档缺失按预期失败。
- Green：新增 operation packet 和级联文档。
...
## 2026年7月8日 - Target Repo Repair Goal Proposal Package v0.1

### 完成内容

- 新增 `packages/acceptance/src/ai-ide-target-repo-repair-goal-proposal-package.ts`。
- 新增 `repoassure.ai-ide-target-repo-repair-goal-proposal-package.v1`。
- 新增 `ai-ide-target-repo-repair-goal-proposal-package.json` / `ai-ide-target-repo-repair-goal-proposal-package.md` 输出。
- 新增 `pnpm playbook:proposal` 和 `scripts/generate-ai-ide-target-repo-repair-goal-proposal-package.mjs`。
- 新增 package export `@hardening-mcp/acceptance/ai-ide-target-repo-repair-goal-proposal-package`。
- `tests/integration/playbook-e2e-repair-evidence.test.ts` 现在覆盖 `campaign-summary -> playbook -> consume -> decide -> approve -> plan-approved -> evidence -> bundle -> contract -> replay -> proposal`。
- 新增 `docs/adr/0028-target-repo-repair-goal-proposal-package.md` 和 `docs/operations/target-repo-repair-goal-proposal-package-v0.1.md`，并级联更新 README、PRD、SPEC、PLAN、architecture overview、acceptance checklist、testing strategy 和 decision log。

### TDD 记录

- Red：先新增 `tests/unit/ai-ide-target-repo-repair-goal-proposal-package.test.ts`，测试因模块缺失按预期失败。
- Green：实现 proposal package builder、Markdown renderer、writer 和目录发现。
- Red：新增 `tests/integration/playbook-proposal.test.ts`，测试因 `pnpm playbook:proposal` 和脚本不存在按预期失败。
- Green：新增 CLI 脚本、package script、package export、compatibility contract entry 和 type-smoke import。
- Red：扩展 E2E campaign fixture 到 `playbook:proposal`，测试暴露 artifact 清单缺失和排序问题。
- Green：补齐 E2E proposal artifact 断言。
- Red：更新 `tests/unit/project-structure.test.ts`，要求 ADR-0028、operation 文档和级联记录存在；测试因文档缺失按预期失败。
- Green：新增 ADR、operation packet 和级联文档。
...
## 2026年7月8日 - Target Repo Repair Goal Authorization Receipt v0.1

### 完成内容

- 新增 `packages/acceptance/src/ai-ide-target-repo-repair-goal-authorization-receipt.ts`。
- 新增 `repoassure.ai-ide-target-repo-repair-goal-authorization-receipt.v1`。
- 新增 `ai-ide-target-repo-repair-goal-authorization-receipt.json` / `ai-ide-target-repo-repair-goal-authorization-receipt.md` 输出。
- 新增 `pnpm playbook:authorize` 和 `scripts/generate-ai-ide-target-repo-repair-goal-authorization-receipt.mjs`。
- 新增 package export `@hardening-mcp/acceptance/ai-ide-target-repo-repair-goal-authorization-receipt`。
- `tests/integration/playbook-e2e-repair-evidence.test.ts` 现在覆盖 `campaign-summary -> playbook -> consume -> decide -> approve -> plan-approved -> evidence -> bundle -> contract -> replay -> proposal -> authorization receipt`。
- 新增 `docs/adr/0029-target-repo-repair-goal-authorization-receipt.md` 和 `docs/operations/target-repo-repair-goal-authorization-receipt-v0.1.md`，并级联更新 README、PRD、SPEC、PLAN、architecture overview、acceptance checklist、testing strategy 和 decision log。

### TDD 记录

- Red：先新增 `tests/unit/ai-ide-target-repo-repair-goal-authorization-receipt.test.ts`，测试因模块缺失按预期失败。
- Green：实现 authorization receipt builder、Markdown renderer、writer 和目录发现。
- Red：新增 `tests/integration/playbook-authorization.test.ts`，测试因 `pnpm playbook:authorize` 和脚本不存在按预期失败。
- Green：新增 CLI 脚本、package script、package export、compatibility contract entry 和 type-smoke import。
- Red：扩展 E2E campaign fixture 到 `playbook:authorize`，测试因命令缺失按预期失败。
- Green：补齐 E2E authorization receipt 生成和 artifact 断言。
- Red：更新 `tests/unit/project-structure.test.ts`，要求 ADR-0029、operation 文档和级联记录存在；测试因文档缺失按预期失败。
- Green：新增 ADR、operation packet 和级联文档。
...
## 2026年7月9日 - Authorized Target Repo Repair Goal Task Package v0.1

### 完成内容

- 新增 `packages/acceptance/src/ai-ide-authorized-target-repo-repair-goal-task-package.ts`。
- 新增 `repoassure.ai-ide-authorized-target-repo-repair-goal-task-package.v1`。
- 新增 `ai-ide-authorized-target-repo-repair-goal-task-package.json` / `ai-ide-authorized-target-repo-repair-goal-task-package.md` 输出。
- 新增 `pnpm playbook:target-repair-goal` 和 `scripts/generate-ai-ide-authorized-target-repo-repair-goal-task-package.mjs`。
- 新增 package export `@hardening-mcp/acceptance/ai-ide-authorized-target-repo-repair-goal-task-package`。
- `tests/integration/playbook-e2e-repair-evidence.test.ts` 现在覆盖 `campaign-summary -> playbook -> consume -> decide -> approve -> plan-approved -> evidence -> bundle -> contract -> replay -> proposal -> authorization receipt -> target repair goal task package`。
- 新增 `docs/adr/0030-authorized-target-repo-repair-goal-task-package.md` 和 `docs/operations/authorized-target-repo-repair-goal-task-package-v0.1.md`，并级联更新 README、PRD、SPEC、PLAN、architecture overview、acceptance checklist、testing strategy 和 decision log。

### TDD 记录

- Red：先新增 `tests/unit/ai-ide-authorized-target-repo-repair-goal-task-package.test.ts`，测试因模块缺失按预期失败。
- Green：实现 task package builder、Markdown renderer、writer 和目录发现。
- Red：新增 `tests/integration/playbook-target-repair-goal.test.ts`，测试因 `pnpm playbook:target-repair-goal` 和脚本不存在按预期失败。
- Green：新增 CLI 脚本、package script、package export、compatibility contract entry 和 type-smoke import。
- Red：扩展 E2E campaign fixture 到 `playbook:target-repair-goal`，测试因命令缺失按预期失败。
- Green：补齐 E2E target repair goal task package 生成和 artifact 断言。
- Red：更新 `tests/unit/project-structure.test.ts`，要求 ADR-0030、operation 文档和级联记录存在；测试因文档缺失按预期失败。
- Green：新增 ADR、operation packet 和级联文档。
...
## 2026年7月9日 - Target Repo Repair Goal Execution Evidence Intake v0.1

### 完成内容

- 新增 `packages/acceptance/src/ai-ide-target-repo-repair-goal-execution-evidence-intake-report.ts`。
- 新增 `repoassure.ai-ide-target-repo-repair-goal-execution-evidence-intake-report.v1`。
- 新增 `ai-ide-target-repo-repair-goal-execution-evidence-intake-report.json` / `ai-ide-target-repo-repair-goal-execution-evidence-intake-report.md` 输出。
- 新增 `pnpm playbook:target-repair-evidence` 和 `scripts/generate-ai-ide-target-repo-repair-goal-execution-evidence-intake-report.mjs`。
- 新增 package export `@hardening-mcp/acceptance/ai-ide-target-repo-repair-goal-execution-evidence-intake-report`。
- `tests/integration/playbook-e2e-repair-evidence.test.ts` 现在覆盖 `campaign-summary -> playbook -> consume -> decide -> approve -> plan-approved -> evidence -> bundle -> contract -> replay -> proposal -> authorization receipt -> target repair goal task package -> target repair evidence intake`。
- 新增 `docs/adr/0031-target-repo-repair-goal-execution-evidence-intake.md` 和 `docs/operations/target-repo-repair-goal-execution-evidence-intake-v0.1.md`，并级联更新 README、PRD、SPEC、PLAN、architecture overview、acceptance checklist、testing strategy 和 decision log。

### TDD 记录

- Red：先新增 `tests/unit/ai-ide-target-repo-repair-goal-execution-evidence-intake-report.test.ts`，测试因模块缺失按预期失败。
- Green：实现 intake report builder、Markdown renderer、writer 和目录发现。
- Red：新增 `tests/integration/playbook-target-repair-evidence.test.ts`，测试因 `pnpm playbook:target-repair-evidence` 和脚本不存在按预期失败。
- Green：新增 CLI 脚本、package script、package export、compatibility contract entry 和 type-smoke import。
- Red：扩展 E2E campaign fixture 到 `playbook:target-repair-evidence`。
- Green：补齐 E2E target repair evidence intake 生成和 artifact 断言。
- Red：更新 `tests/unit/project-structure.test.ts`，要求 ADR-0031、operation 文档和级联记录存在；测试因文档缺失按预期失败。
- Green：新增 ADR、operation packet 和级联文档。
...
- PR #50 首轮 GitHub Quality Gates：通过（1m44s）；最终 PR CI、merge 和 main CI 待收口。
- PR #49 首轮 GitHub Quality Gates：通过（1m38s）；最终 PR CI、合并与 main CI 待收口。
- Review package does not execute commands or close goals。

## 2026年7月13日 - Blocked Goal Recovery Resume Attempt Closure Receipt v0.1

- Red：closure receipt module、CLI、exports、ADR 和文档级联不存在。
- Green：实现 exact review package SHA、strict review package runtime validation、accepted/accepted-risk closure gate、exact risk acknowledgement、typed JSON/Markdown、CLI 和 near-real campaign stage。
- Independent review 首轮发现 fabricated internally consistent review package 可绕过 upstream provenance；TDD 修复要求 exact task package 和 execution intake，并逐层校验 task-to-intake 与 intake-to-review SHA、inventory 和 evidence fields。
- Independent review 复审：无 actionable findings；19 个 focused tests 和 acceptance typecheck 通过。
- `pnpm typecheck`、`pnpm lint`：通过。
- `pnpm test:unit`：62 files，737 tests，通过。
- Integration：33 files，64 tests，通过；E2E：1 passed / 1 skipped。
- Full serial pyramid：96 files passed / 1 skipped；802 tests passed / 1 skipped。
- `pnpm repo:hygiene`、`pnpm release:check`：通过；`pnpm goal:audit`：35/35，通过。
- Boundary：receipt records local evidence only；commands executed and external goal closure remain false。
- PR CI、merge 和 main CI 待收口。
...
## 2026年7月13日 - Parallel Test Runtime Build Isolation v0.1

- Root cause：clean `pnpm test` lacked package/root prebuild；prebuilt full runs then exposed parallel `build:acceptance` writers and moving partial-ESM export failures.
- Red：runtime contract and eight-process integration were 5/5 failed because the coordinator and prebuilt standard test entry did not exist；orphan-owner test then reproduced a deterministic lock timeout after forced process exit.
- Green：新增 source fingerprint、cross-process lock、successful-output state、failure cleanup、atomic orphan quarantine；`build:acceptance` 使用 `node --import tsx` 避免受限环境中的 tsx CLI IPC listener。
- Runtime hardening：Python CLI full artifact test receives a scoped 15-second test budget；real stdio MCP harness uses realistic startup/request budgets while preserving deliberate timeout, PID termination, environment isolation and redaction assertions.
- Focused evidence：build isolation 4/4、runtime/docs contract 3/3、project structure 158/158、real MCP 5/5；typecheck and lint passed.
- Full evidence：implementation-stage standard parallel runs produced two complete passes before realistic MCP scheduling budgets were finalized。Final evidence then produced three consecutive standard file-parallel `pnpm test` passes；each run reported 104 passed files / 1 optional skipped and 850 passed tests / 1 optional skipped，without serialized fallback or shared-dist failures。
- Boundary：no recovery/resume command execution, target repo mutation, publish, launch, customer contact, pricing/spend or visibility change, or commercial/hosted availability claim.
- Independent review first pass：发现旧 success state 可在失败重建写出 partial output 后被错误复用、锁目录与 owner metadata 存在非原子窗口、fingerprint 未覆盖构建实现，以及 PLAN 过早标记完成。
- TDD remediation：新增 previous-success / failed-partial-rebuild regression 和 real fingerprint mutation contract；构建前删除旧 state；锁改为单个原子 `wx` lease-token file 并只允许 owner token 释放；fingerprint 覆盖 build/coordinator scripts；成功状态验证全部 acceptance JavaScript、declaration 和 source-map outputs；Goal 状态保持 delivery in progress，直到 PR/main CI 收口。
- Post-review full-run diagnosis：默认 host-dependent Vitest worker 数在全量套件中放大 nested CLI/MCP children，导致 6 files / 11 tests 出现 timeout，但没有 shared-dist export failure。新增 RED contract 后将 file parallelism 明确保持开启并限制为 4 workers；不采用 serialized fallback 或 blanket timeout increase。
