...
Public Website Custom Domain Deployment v0.1 的当前测试范围限于 structure-level tests 和外部状态记录：`project-structure.test.ts` 守护 `docs/operations/public-website-custom-domain-deployment-v0.1.md`、README、acceptance checklist、testing strategy、dev log 和 blockers 的级联，要求记录 `blocked_dns_cname_not_set`、Cloudflare Pages project `repoassure-preview`、custom domains `repoassure.com` / `www.repoassure.com`、latest deployment URL、Pages custom domain API accepted both domain bindings、Cloudflare verification result `CNAME record not set`、DNS API result `Authentication error`、所需 CNAME 记录和 non-authorization boundary。HTTPS、页面内容、语言切换、desktop/mobile smoke 和 forbidden-claim custom-domain verification 必须等 DNS CNAME 生效后执行；当前测试不得把 pending domain binding 视为 public launch 或 SaaS/Team Cloud/Enterprise/hosted dashboard availability。
